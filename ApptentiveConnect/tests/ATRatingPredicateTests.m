...
- (void)testPredicateStrings {
	...
	XCTAssertEqualObjects([ATAppRatingFlow_Private predicateStringForPromptLogic:[self allAndLogic] withPredicateInfo:info hasError:nil], @"((significantEvents >= significantEventsBeforePrompt) AND (appUses >= usesBeforePrompt))", @"Predicate should not contain disabled parameter.");
	XCTAssertEqualObjects([ATAppRatingFlow_Private predicateStringForPromptLogic:[self allOrLogic] withPredicateInfo:info hasError:nil], @"((significantEvents >= significantEventsBeforePrompt) OR (appUses >= usesBeforePrompt))", @"Predicate should not contain disabled parameter.");
	
	
	info.daysBeforePrompt = 1;
	info.significantEventsBeforePrompt = 0;
	XCTAssertEqualObjects([ATAppRatingFlow_Private predicateStringForPromptLogic:[self allAndLogic] withPredicateInfo:info hasError:nil], @"((now >= nextPromptDate) AND (appUses >= usesBeforePrompt))", @"Predicate should not contain disabled parameter.");
	XCTAssertEqualObjects([ATAppRatingFlow_Private predicateStringForPromptLogic:[self allOrLogic] withPredicateInfo:info hasError:nil], @"((now >= nextPromptDate) OR (appUses >= usesBeforePrompt))", @"Predicate should not contain disabled parameter.");
...
- (void)testOrPredicates {
	...
	XCTAssertFalse([ATAppRatingFlow_Private evaluatePredicate:missingClause withPredicateInfo:info], @"Predicate should not be true.");
	info.appUses = 11;
	...
	XCTAssertTrue([ATAppRatingFlow_Private evaluatePredicate:missingClause withPredicateInfo:info], @"Predicate should be true.");
	info.appUses = 3;
	missingClause = [ATAppRatingFlow_Private predicateForPromptLogic:[NSDictionary dictionaryWithObjectsAndKeys:[NSArray arrayWithObjects:@"days", @"uses", nil], @"or", nil] withPredicateInfo:info];
...
- (void)testNilInfo {
	...
	XCTAssertNotNil(predicate, @"Should be able to validate predicate with nil info.");
}
@end
