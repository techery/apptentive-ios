...
class PictureManager {
	...
	init() {
		if let picturesFolderPath = Bundle.main.pathForResource("Pictures", ofType: nil) {
			let enumerator = FileManager.default.enumerator(atPath: picturesFolderPath)
			
			while let element = enumerator?.nextObject() as? String {
				let pictureURL = URL.fileURL(withPath: NSString(string: picturesFolderPath).appendingPathComponent(element as String))
	...
	private func indexOfFavorite(_ picture: Picture) -> Int? {
...
class PictureDataSource {
	...
	func imageNameAtIndex(_ index: Int) -> String {
		return try! self.pictures[index].URL.deletingPathExtension!.lastPathComponent!
	}
